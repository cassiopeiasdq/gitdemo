define(["parabola", "jquery", "jquery-cookie"], function(parabola, $){
    ...
    function index(){
        $(function(){
		   ...
		   $.ajax({
			url: "data/sp_shoesdata.json",
		 //    dataType: "json",
			success: function(arr){
			   
				 for(var i = 0; i < arr.length; i++){
					 var node = 
					 		$(`<a class = 'sp_shoesa' href = "">
							 ${arr[i].name}
						 	</a>`)
					 node.appendTo("#in_sport .in_left .sp_shoes");
				 }
		...
		$.ajax({
		 ...
		 //    dataType: "json",
			success: function(arr){
				 ...
				 for(var i = 0; i < arr.length; i++){
					 var node = 
					 		$(`<a href = "" target = "_blank" style="background:url(${arr[i].img})">
							 <span class = "tit1">${arr[i].tit1}</span>
							 <span class = "tit2">${arr[i].tit2}</span>
							 <span class = "tit3">${arr[i].tit3}</span>
						 	</a>`)
					 node.appendTo("#list2");
				 }
